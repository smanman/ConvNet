...
from unetr_pp.network_architecture.fdg_pet_ct.transformerblock_pet import TransformerBlock
from unetr_pp.network_architecture.dynunet_block import get_conv_layer, UnetResBlock, UnetOutBlock
...
class SCS3D(nn.Module):
    ...
    def forward(self, x1, x2):
        out_x = torch.cat([x1, x2], dim=1)
        out = self.bn1(self.conv1(out_x))
        out = F.relu(out)
        ...
        excitation = self.sigmoid(excitation).unsqueeze(2).unsqueeze(3).unsqueeze(4)
    ...
    def __init__(
        ...
        if conv_decoder == True:
            ...
            for j in range(depth):
                stage_blocks.append(TransformerBlock(
                    input_size=out_size,
...
class UnetrPPEncoder(nn.Module):
    def __init__(self,
                 ...
                 num_heads=4, 
                 spatial_dims=3, 
                 in_channels=2,  # 对PET-CT修改为2个输入通道
                 dropout=0.0, 
