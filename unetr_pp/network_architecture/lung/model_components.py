...
class SCS3D(nn.Module):
    ...
    def forward(self, x1, x2):
        ...
        out = out * excitation.expand_as(out)
        out = out + x2
        return out
...
class ux_block(nn.Module):
    ...
    def __init__(self, dim, drop_path=0., layer_scale_init_value=1e-6):
        ...
        # self.dwconv = nn.Conv3d(dim, dim, kernel_size=3, padding=3, groups=dim)
        ...
        self.SE3D = SEWeightModule3D(dim)
    ...
    def forward(self, x):
        ...
        x = x.permute(0, 4, 1, 2, 3)
        drop_path_x = self.drop_path(x)
        x = drop_path_x * self.SE3D(drop_path_x)
        x = input + x
        return x
...
class UnetrPPEncoder(nn.Module):
    def __init__(self, input_size=[32 * 48 * 48, 16 * 24 * 24, 8 * 12 * 12, 4 * 6 * 6], dims=[32, 64, 128, 256],
                 ...
                 dropout=0.0, transformer_dropout_rate=0.1, drop_path_rate=0., layer_scale_init_value=1e-6,
        ...
        for i in range(4):
            ...
            # self.skips.append(nn.Sequential(*skip_blocks))
        cur += depths[i]
