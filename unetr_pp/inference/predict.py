...
def preprocess_multithreaded(trainer, list_of_lists, output_files, num_processes=2, segs_from_prev_stage=None):
    ...
    assert isinstance(trainer, Trainer_acdc) or isinstance(trainer, Trainer_synapse) or isinstance(trainer, Trainer_tumor) or isinstance(trainer, Trainer_lung) or isinstance(trainer, Trainer_pet) or isinstance(trainer, Trainer_lung)
    q = Queue(1)
    processes = []
