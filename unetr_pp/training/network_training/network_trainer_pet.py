...
class NetworkTrainer_pet(object):
    ...
    def manage_patience(self):
        ...
        if self.patience is not None:
            ...
            if self.val_eval_criterion_MA > self.best_val_eval_criterion_MA:
                self.best_val_eval_criterion_MA = self.val_eval_criterion_MA
                self.print_to_log_file("发现新的最佳模型，当前评估指标: %.4f" % self.val_eval_criterion_MA)
                self.print_to_log_file("保存最佳模型 model_best.model ...")
                if self.save_best_checkpoint: 
                    self.save_checkpoint(join(self.output_folder, "model_best.model"))
                self.print_to_log_file("最佳模型保存完成")
