...
from unetr_pp.training.network_training.network_trainer_btcv import NetworkTrainer_btcv
...
class Trainer_btcv(NetworkTrainer_btcv):
    def __init__(self, plans_file, fold, output_folder=None, dataset_directory=None, batch_dice=True, stage=None,
        ...
        """
        super(Trainer_btcv, self).__init__(deterministic, fp16)
    ...
    def run_training(self):
        ...
        super(Trainer_btcv, self).run_training()
    ...
    def predict_preprocessed_data_return_seg_and_softmax(self, data: np.ndarray, do_mirroring: bool = True,
        ...
        if not hasattr(self, '_first_inference'):
        ...
        # Skip first inference time due to potential initialization overhead
        if not self._first_inference:
            self._inference_times.append(end_time - start_time)
            if len(self._inference_times) % 10 == 0:  # Print average every 10 inferences
                avg_time = sum(self._inference_times) / len(self._inference_times)
                self.print_to_log_file(f"Average inference time: {avg_time:.4f} seconds")
        else:
    ...
    def save_checkpoint(self, fname, save_optimizer=True):
        super(Trainer_btcv, self).save_checkpoint(fname, save_optimizer)
        info = OrderedDict()
