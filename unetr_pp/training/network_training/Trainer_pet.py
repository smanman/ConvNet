...
from unetr_pp.training.network_training.network_trainer_pet import NetworkTrainer_pet
...
class Trainer_pet(NetworkTrainer_pet):
    def __init__(self, plans_file, fold, output_folder=None, dataset_directory=None, batch_dice=True, stage=None,
        ...
        """
        super(Trainer_pet, self).__init__(deterministic, fp16)
    ...
    def run_training(self):
        ...
        super(Trainer_pet, self).run_training()
        total_training_end_time = time.time()
        total_training_time = total_training_end_time - total_training_start_time
        self.print_to_log_file("Total training time: %.2f seconds" % total_training_time)
    ...
    def process_plans(self, plans):
        ...
        self.num_input_channels = plans['num_modalities']  # for PET-CT, this is 2 (PET and CT)
    ...
    def preprocess_predict_nifti(self, input_files: List[str], output_file: str = None,
        ...
        self.print_to_log_file("Inference time: %.2f seconds" % inference_time)
    ...
    def validate(self, do_mirroring: bool = True, use_sliding_window: bool = True, step_size: float = 0.5,
        ...
        _ = aggregate_scores(pred_gt_tuples, labels=list(range(self.num_classes)),
                             ...
                             json_author="Evaluation Script",
                             json_task=task, num_threads=default_num_threads)

        if run_postprocessing_on_folds:
            # in the old unetr_pp we would stop here...
            self.print_to_log_file("determining postprocessing")
            determine_postprocessing(self.output_folder, self.gt_niftis_folder, validation_folder_name,
                                     final_subf_name=validation_folder_name + "_postprocessed", debug=debug)
            # after this the final predictions...

            # detemining postprocesing on a per-fold basis...
    ...
    def save_checkpoint(self, fname, save_optimizer=True):
        super(Trainer_pet, self).save_checkpoint(fname, save_optimizer)
        info = OrderedDict()
    ...
    def maybe_update_lr(self):
        ...
        if self.lr_scheduler is not None:
            ...
            if isinstance(self.lr_scheduler, lr_scheduler.ReduceLROnPlateau):
                # 使用移动平均验证损失更新学习率调度器，这对于PET-CT的稳定训练很重要
    ...
    def update_eval_criterion_MA(self):
        ...
        if self.val_eval_criterion_MA is None:
            ...
            if len(self.all_val_eval_metrics) == 0:
                # 使用alpha * old - (1 - alpha) * new因为new在这种情况下是验证损失，值越小越好
    ...
    def manage_patience(self):
        """
        根据训练过程中的进展管理耐心值，决定是否继续训练
        """
        # 更新耐心值
        continue_training = True
        if self.patience is not None:
            # 如果best_MA_tr_loss_for_patience和best_epoch_based_on_MA_tr_loss尚未初始化，则初始化它们
            ...
            # 检查当前epoch是否根据验证标准的移动平均值是最佳epoch。如果是，则保存'最佳'模型
            # 不要将此用于验证。这仅用于测试集预测。
            ...
            # 现在看看训练损失的移动平均值是否有所改善。如果有，则重置耐心值，否则增加耐心值
    ...
    def on_epoch_end(self):
        ...
        在每个epoch结束时执行的操作
        """
        self.finish_online_evaluation()  # 完成在线评估，更新self.all_val_eval_metrics

        self.plot_progress()  # 绘制训练进度图表
